...
#MAke suer you are adding "amazon river" to the list
echo -e "Amazon River\n" > rivers.txt
echo -e "Nile River\n" >> rivers.txt
...
sed -i 's/Amazon/Ohio/g' rivers.txt > new_rivers.txt
...
#Compare the original file and the modified
diff rivers.txt new_rivers_v3.txt


#One-Liner
touch rivers.txt && echo -e "Amazon River,\n Nile River,\n Yangtze River,\n Potomac River,\n Danube River,\n Potomac River" > rivers.txt && sort rivers.txt | uniq > rivers2.txt | sed 's/Amzon/Ohio/g' rivers3.txt && diff rivers.txt rivers3.txt
